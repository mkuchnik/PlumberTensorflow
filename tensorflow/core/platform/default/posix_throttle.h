...
#ifndef TENSORFLOW_CORE_PLATFORM_CLOUD_POSIX_THROTTLE_H_
#define TENSORFLOW_CORE_PLATFORM_CLOUD_POSIX_THROTTLE_H_
...
/**
 * PosixThrottleConfig is used to configure the PosixThrottle.
 */
struct PosixThrottleConfig {
  /**
   * enabled is true if PosixThrottle should throttle requests, false otherwise.
   */
  bool enabled = false;

  /**
   * token_rate is the number of tokens accrued every second that can be used
   * for making requests to the Posix service.
   */
  int64 token_rate = 100000;  // Approximately 800 MBits/second bandwidth-only.

  /**
   * bucket_size is the maximum number of available tokens the PosixThrottle can
   * accrue.
   */
   ...
   * after the PosixThrottle is constructed.
   */
  int64 initial_tokens = 0;
};

/**
 * PosixThrottle is used to ensure fair use of the available Posix capacity.
 *
 * PosixThrottle operates around a concept of tokens. Tokens are consumed when
 * making requests to the Posix service. Tokens are consumed both based on the
 * number of requests made, as well as the bandwidth consumed (response sizes).
 *
 * PosixThrottle is thread safe and can be used from multiple threads.
 */
class PosixThrottle {
 public:
  /**
   * Constructs a PosixThrottle.
   */
  explicit PosixThrottle(EnvTime* env_time = nullptr);

  /**
   * AdmitRequest updates the PosixThrottle to record a request will be made.
   *
  ...
  /**
   * RecordResponse updates the PosixThrottle to record a request has been made.
   *
  ...
  /**
   * SetConfig sets the configuration for PosixThrottle and re-initializes state.
   *
   * After calling this, the token pool will be config.initial_tokens.
   */
  void SetConfig(PosixThrottleConfig config);
  ...
  /**
   * The maximum bandwidth that can be sustained in kibibytes/second.
   *
   * In other words, outputs 1024 * bytes / second
   */
  inline int64 available_max_bandwidth_kibytes_sec() TF_LOCKS_EXCLUDED(mu_) {
    mutex_lock l(mu_);
    if (!config_.enabled) {
      return -1;
    } else {
      return config_.token_rate;
    }
  }
 ...
 private:
   ...
   * the internal state of the PosixThrottle was updated. This is important when
  ...
  PosixThrottleConfig config_ TF_GUARDED_BY(mu_);
};

}  // namespace tensorflow

#endif  // TENSORFLOW_CORE_PLATFORM_CLOUD_POSIX_THROTTLE_H_
