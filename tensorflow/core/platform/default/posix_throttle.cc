...
void PosixThrottle::RecordResponse(size_t num_bytes) {
  mutex_lock l(mu_);
  UpdateState();
  available_tokens_ -= request_bytes_to_tokens(num_bytes);
}

void PosixThrottle::SetConfig(PosixThrottleConfig config) {
  mutex_lock l(mu_);
  config_ = config;
...
void PosixThrottle::UpdateState() {
